...
;;	  (fire 0 0) ;; t = t->t+1
	  ...
	  (let* ([cs-x (- (vm-oport vm 2))]
			 ...
			 [cs-theta (atan cs-y cs-x)]
			 [ts-x (+ cs-x (vm-oport vm 4))]
			 [ts-y (+ cs-y (vm-oport vm 5))]
			 [ts-r (hypot ts-x ts-y)]
			 [ts-theta (atan ts-y ts-x)]
		...
		(push! g:cs-trace (cons cs-x cs-y))
		(push! g:target-trace (cons ts-x ts-y))
