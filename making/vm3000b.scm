...
(case *scenario-id*
  ...
  [(3004)
   ...
   (set! target-perigee-r 8357000)
   (set! target-apogee-x 0)
   (set! target-apogee-y -12800000)
   (set! target-apogee-r 0)
   ...
   (set! target-semi-major-axis (/ (+ (hypot target-perigee-x target-perigee-y)
									  (hypot target-apogee-x target-apogee-y))
								   2))
   (set! target-cycle (* 2 pi (sqrt (/ (expt target-semi-major-axis 3) mu))))
   ]
  )
...
;	(set! target-r target-r1)
	...
	(set! vcir2 (sqrt (/ mu target-r*)))
	(set! delta-v1 (* vcir1 (- (sqrt (/ (* 2 target-r*) (+ cs-r2 target-r*))) 1)))
;		   [delta-v1 (- (* vcir1 (sqrt (/ (* 2 target-r1) (+ cs-r2 target-r1)))) delta)]
	(set! delta-v2 (* vcir2 (- 1 (sqrt (/ (* 2 cs-r2) (+ cs-r2 target-r*))))))
