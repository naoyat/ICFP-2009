...
;		  (format #t "// r=~a th=~a dx=~a dy=~a\n" r (* 180/pi cs-d-theta) dx dy)
		  (case ch
			...
			[(#\Space)
			 (call/cc
			  (lambda (break)
				(let ((delta-vx 0) (delta-vy 0) (skip-local #f))
				(dotimes (j 64)
				  (dotimes (i 16)
					(unless skip-local
					  ...
					  (let* ([cs-th (atan cs-y cs-x)]
						...
						(when last-cs-x
						  (let* ([vx (- cs-x last-cs-x)]
								 ...
								 [anti-gx (* g (cos cs-th))]
								 [anti-gy (* g (sin cs-th))]
								 ;[dnorm (/ (hypot vx vy) (hypot dirx diry))]
								 ;[dist (/ (hypot dirx diry) (hypot vx vy))]
								 ;[dnorm (if (< 1.1 dist) (/ 1.1 dist) 1.0)]
								 )
							...
							(if (< dist 10000)
								#;(let* ([dh (hypot dirx diry)]
									   [dvx (- (/ dirx dh) vx)]
									   [dvy (- (/ diry dh) vy)]
									   )
								  (format #t "dist=~a, mag=~a, vh=~a firing (~a, ~d):~a\n"
										  dist mag vh
										  dvx dvy (hypot dvx dvy) )
								  (vm-step vm dvx dvy)
								  (dotimes (i 10) (vm-step vm 0 0))
								  )
								(let (
