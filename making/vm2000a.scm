...
(vm-load vm "bin2.obf")
;(define *scenario-id* 2003)
(define *scenario-id* 2002) ;; passed
;(define *scenario-id* 2001) ;; bimyou
...
;		   [delta-v1 (- (* vcir1 (sqrt (/ (* 2 target-r1) (+ cs-r2 target-r1)))) delta)]
	...
	(let* ([TH (* pi (sqrt (/ (expt (+ cs-r2 target-r2) 3) 8 mu)))]
				...
				(dotimes (j 64)
				  (dotimes (i 16)
					...
					(let* ([score (vm-oport vm 0)]
						   ...
						   [dist (hypot (vm-oport vm 4) (vm-oport vm 5))]
...
;									(set! delta-v2 (- vcir2 v))
									...
									(let ([dvx (* delta-v2 (cos th))]
									  ...
									  (when (< (hypot (+ vx dvx) (+ vy dvy)) (hypot vx vy))
										(set! dvx (- dvx))
										(set! dvy (- dvy))
										;(set! dvx (- (* -1 vcir2 (cos th)) vx))
									    ;(set! dvy (- (* -1 vcir2 (sin th)) vy))
