...
;;				  (pp-stat stat last-stat)
				  (let* (;;[cx (stat-cs-x stat)]
						 ;;[cy (stat-cs-y stat)]
						 [cs-dir (stat-cs-theta stat)]
...
;						  (fire (- dx) (- dy))
						  ...
						  (let* ([cs-dir (stat-cs-theta stat)]
							...
							(fire (+ (* (+ (- dist-go) vdiff) (cos cs-go-dir))
									 (* (+ (- dist-up) 0) (cos cs-dir))
									 )
								  (+ (* (+ (- dist-go) vdiff) (sin cs-go-dir))
									 (* (+ (- dist-up) 0) (sin cs-dir))
									 ))
							)
;						  (fire (* -1 dist-go (cos cs-go-dir))
;								(* -1 dist-go (sin cs-go-dir)))
						  (fire 0 0)
						  (break))
						(let ([dx (stat-ds-x stat)]
							  [dy (stat-ds-y stat)]
				   ...
				   #;(when (< 300 (abs dist-go))
					  (format #t ">> dist-go: ~a, then ~a\n" dist-go (- dist-go))
