...
(define g:gs-trace '())
(define g:moon-trace '())
(define g:targets-trace (make-vector 11 '()))
...
(define (fire dvx dvy)
  ...
  (push! g:gs-trace (cons (stat-gs-x stat) (stat-gs-y stat)))
  (push! g:moon-trace (cons (stat-moon-x stat) (stat-moon-y stat)))
  (dotimes (k 11)
	(push! (vector-ref g:targets-trace k)
		   (cons (stat-ts-x k stat) (stat-ts-y k stat))))
  )
...
(define (analyze)
  (define (gravity x y)
	(let* ([earth-r (hypot x y)]
	  ...
	  (cons (* earth-g (cos earth-th))
			(* earth-g (sin earth-th)) )))
  (define (gravity-sum x y moon-x moon-y)
	(let* ([earth-r (hypot x y)]
		   [earth-th (atan y x)]
		   [moon-r (hypot moon-x moon-y)]
		   [moon-th (atan moon-y moon-x)]
		   [earth-g (/ mu earth-r earth-r)]
		   [moon-g (/ mu-moon moon-r moon-r)])
	  (cons (+ (* earth-g (cos earth-th)) (* moon-g (cos moon-th)))
			(+ (* earth-g (sin earth-th)) (* moon-g (sin moon-th))) )))
  (define (calc-moon-v1 x0 y0 x1 y1)
	(let* ([dx (- x1 x0)]
		   [dy (- y1 y0)]
		   [g0 (gravity x0 y0)]
		   [g1 (gravity x1 y1)]
	  ...
	  (cons v1x v1y)))
  (define (calc-v1 x0 y0 x1 y1 moon0x moon0y moon1x moon1y)
	(let* ([dx (- x1 x0)]
		   [dy (- y1 y0)]
		   [g0 (gravity-sum x0 y0 moon0x moon0y)]
		   [g1 (gravity-sum x1 y1 moon1x moon1y)]
		   [v0x (- dx (car g0))]
		   [v0y (- dy (cdr g0))]
		   [v1x (+ v0x (/ (+ (car g0) (car g1)) 2))]
		   [v1y (+ v0y (/ (+ (cdr g0) (cdr g1)) 2))])
  ...
  (let ([m0x (stat-moon-x last-stat)]
		[m0y (stat-moon-y last-stat)]
		[m1x (stat-moon-x stat)]
		[m1y (stat-moon-y stat)])
	(let1 moon-v1 (calc-moon-v1 m0x m0y m1x m1y)
	  (format #t "moon-v1: ~a\n" moon-v1)
	  (let loop ((cs-x 0) (cs-y 0) (cs-vx 0) (cs-vy 0)
				 (moon-x m1x) (moon-y m1y) (moon-vx (car moon-v1)) (moon-vy (cdr moon-v1))
				 (fuel-x 0) (fuel-y 0) (fuel-vx 0) (fuel-vy 0)
				 )
		)
	  )))
...
;			(set! fire3-at (round->exact (- goal-at (/ (cycle-period (/ (+ ts-min-r middle-orbit-r) 2)) 2)) ))
	  ...
	  (set! *display-scale* (/ 0.9 (max cs-max-r ts-max-r *earth-r* 384000000 middle-orbit-r)))
	  )))

|#
;(set! *display-scale* (/ 0.9 500000000))
;(set! *display-scale*  (/ 0.9 80000000))
...
;						 [ts-vx (- (stat-ts-x 0 stat) (stat-ts-x last-stat))]
	;					 [ts-vy (- (stat-ts-y 0 stat) (stat-ts-y last-stat))]
...
;	  (draw-string* -1 0.95 (format "t=~d d=~a" t (stat-dist stat)))
	  (draw-string* -1 0.95 (format "t=~d fuel=~a" t (stat-fuel stat)))
	...
	;; moon
	(gl-color 0.5 0.5 0.1)
	(draw-trace g:moon-trace)
	(draw-circle (stat-moon-x stat) (stat-moon-y stat) *moon-r* 'fill)

	;; fuel sta
	(gl-color 1.0 0.1 0.1)
	(draw-trace g:gs-trace)
	(draw-char (stat-gs-x stat) (stat-gs-y stat) #\F)
	...
	(dotimes (k 11)
	  (draw-trace (vector-ref g:targets-trace k))
	  ;; (draw-char (stat-ts-x k stat) (stat-ts-y k stat) (integer->char (+ 48 k)))) ;;#\T);)
	  (draw-cross (stat-ts-x k stat) (stat-ts-y k stat) 0.01)
	  )
